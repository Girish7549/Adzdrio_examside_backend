...
export const createOption = async (question_id, option_text, is_correct, status) => {
  try {
    const query = 'INSERT INTO options (questions_id, option_text, is_correct, status) VALUES (?, ?, ?, ?)';
    const result = await db.query(query, [question_id, option_text, is_correct, status]);
