...
export const getAllCardByUserIdModel = async (userId) => {
  try {
    const query = `
      SELECT carts.*, product.product_image, product.product_name, product.product_price, carts.quantity, carts.final_price
      FROM carts 
      JOIN product ON carts.product_id = product.product_id 
      WHERE carts.user_id = ?`;
    ...
    if (result.length === 0) {
      return null; // If no carts are found for the user
    }

    // Calculate the total price by summing the final_price of each item in the cart
    const totalPrice = result.reduce((total, item) => {
      // Ensure final_price is treated as a number
      const price = parseFloat(item.final_price) || 0; // Ensure it's a valid number, default to 0 if not
      return total + price; // Sum up the final_price for each cart item
    }, 0);

    // Format the totalPrice to two decimal places and return it as a string
    const formattedTotalPrice = totalPrice.toFixed(2); // Format to two decimal places

    // Return the result along with formatted totalPrice
    return { cartItems: result, totalPrice: formattedTotalPrice };
