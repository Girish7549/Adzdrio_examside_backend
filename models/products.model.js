...
export const createProductModal = async (
  productName,productPrice,productDetails,productDescription,weight,storeInfo,unit,
  refundable,exchangeable,imageUrls,productType,tags,manufacturingDate,expiryDate,sku,
  status,categoryId,subcategoryId,brandId,discountPrice,weightType,stockType,stock,minWeight,foodPreference
) => {
  try {
    const query = `
      INSERT INTO product (
    ...
    const [result] = await db.query(query, [
      productName,productPrice,productDetails,productDescription,weight,storeInfo,unit,refundable,exchangeable,
      JSON.stringify(imageUrls),productType,tags,manufacturingDate,expiryDate,sku,status,categoryId,subcategoryId,
      brandId,discountPrice,weightType,stockType,stock,minWeight,foodPreference,
    ]);
...
export const getAllProductAdminModal = async () => {
  try {
    const query = `
      SELECT 
        product.*,
...
export const updateProductModel = async (
  ...
  try {
    const query = `
      UPDATE product SET 
        product_name = ?, product_price = ?, product_details = ?, product_description = ?, weight = ?, 
...
export const getSimilarProductModel = async (productId) => {
  try {
    ...
    const [productResult] = await db.query('SELECT category_id, tags FROM product WHERE product_id = ?', [productId]);
