...
export const editBussinessStatusByIdModel = async (businesId, status) => {
    try {
        const query =  'UPDATE businesses SET status = ? WHERE business_id = ?';
        const [result] = await db.query(query, [businesId,status]);
        return result.length === 0 ? null : result;
    } catch (error) {
        console.log('business model', error);
        throw new Error("DB Model Error");
