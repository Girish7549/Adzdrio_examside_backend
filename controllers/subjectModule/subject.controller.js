import { createSubject, getAllSubjects, getSubjectById, updateSubjectById, deleteSubjectById, getSubjectUnitChapterModal } from '../../models/subject.modal.js';
...
export const updateSubjectController = async (req, res) => {
  try {
    const { subjectId } = req.params;
    const { subjectName, description, status } = req.body;

    const result = await updateSubjectById(subjectId, subjectName, description, status);
