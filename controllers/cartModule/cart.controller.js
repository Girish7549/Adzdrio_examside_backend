...
export const getAllCartByUserId = async (req, res) => {
  try {
      ...
      if (!userId) {
          return res.status(400).json({ message: "Please Provide User Id or Invalid" });
      }

      // Fetch the cart details along with the total price
      const results = await getAllCardByUserIdModel(userId);
      ...
      // Send both the cart items and the calculated total price in the response
      return res.status(200).json(results);
  } catch (error) {
