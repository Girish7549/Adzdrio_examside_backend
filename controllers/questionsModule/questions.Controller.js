import {createQuestionModal,getAllQuestionsModal, getQuestionByIdModal, updateQuestionModal, deleteQuestionModal, getAllQuestionsWithTheirOptionsModal, getQuestionsWithOptionsByTopicId} from '../../models/questions.modal.js';

export const createQuestion = async (req, res) => {
  try {
    const {questionsName, shift, topicId, status, difficulty, explanation, chapterId, shiftId} = req.body;

    if (!questionsName || !shift ||  !status || !difficulty || !explanation ) {
      return res.status(400).json({ message: "All fields are required!" });
    } 
    const result = await createQuestionModal(questionsName, shift, topicId, status, difficulty, explanation, chapterId, shiftId);
...
export const getAllQuestions = async (req, res) => {
  try {
    const questions = await getAllQuestionsModal();
...
export const updateQuestion = async (req, res) => {
  try {
    const { id } = req.params;
    const {questionsName, shift, topicId, status, difficulty, explanation, chapterId} = req.body; // Assuming JSON payload
    const result = await updateQuestionModal(id, questionsName, shift, topicId, status, difficulty, explanation, chapterId);
