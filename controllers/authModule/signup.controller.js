...
export const verifyOtp = async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0 && otp === rows[0].otp) {
      ...
      const [rewards] = await db.query("SELECT points FROM wallets WHERE user_id = ?", [user[0].id]);
      ...
      return res
        ...
        .json({
          ...
          rewards: rewards.length > 0 ? rewards : [], // Return all wishlists or empty array if none
        });
    } else {
